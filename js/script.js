...
var chat = {
	...
	init : function() {
		// Using the defaultext jQuery plugin, included at the bottom:
		...
		$('#loginForm').submit(function() {
			...
			$.tzPOST('login', $(this).serializes(), function(r) {
				working = false;
		...
		(function getUsersTimeoutFuntion() {
			chat.getUsers(getUsersTimeoutFuntion);
		})();
	},

	// The login method hides dipslays the 
	...
	render : function(template, params) {
		...
		addChatLine : function(params) {
			...
			if (params.time) {
				...
				// object and later output it in the user's timezone. JavasScript
				// Jinternally converts it for us.
			...
			// reinitialise teh jScrollPane plugin:

			chat.data.jspAPI.reinitialise();
			chat.data.jspAPI.scrollToBottom(true);
		},

		// This method request the latest chats
		...
		getChats : function(callback) {
			$.tzGET('getChats', {lastID: chat.data.lastID}, function(r) {
				...
				if (r.chats.length) {
					...
					// If no chats were received, inrement
		...
		// Requesting a list with the users.
		...
		getUsers : function(callback) {
			$.tzGET('getUsers', function(r) {
				...
				uers.push('<p class="count">' +message+ '</p>');

				$('#chatUsers').html(users.joing(''));
